...
__vue_options__.__file = "/Users/acathur/Developments/nat/nat-explorer/src/views/sensor/accelerometer.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-011af95c"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
	...
	methods: {
		getAcceleration: function getAcceleration() {
			_natjs2.default.accelerometer.get(function (err, ret) {
				if (err) {
		...
		watchAcceleration: function watchAcceleration() {
			var _this = this;

			var times = 0;

			_natjs2.default.accelerometer.watch(function (err, ret) {
				times++;

				if (err) {
					_natjs2.default.toast('[ERROR] TIMES: ' + times + '\n' + JSON.stringify(err));
					return;
				}

				_this.result = 'TIMES: ' + times + '\n\nx: ' + ret.x + '\ny: ' + ret.y + '\nz: ' + ret.z;
			});
		},
		clearWatchAcceleration: function clearWatchAcceleration() {
			_natjs2.default.accelerometer.clearWatch(function (err) {
				if (err) {
					_natjs2.default.toast('[ERROR] ' + JSON.stringify(err));
					return;
				}
				_natjs2.default.toast('cleared');
			});
		}
	}
};
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('status-bar'), _c('nav-bar'), _c('scroller', {
    ...
    staticClass: ["case"],
    on: {
      "click": _vm.getAcceleration
    }
  }, [_vm._m(0), _c('text', {
    staticClass: ["code"]
  }, [_vm._v("Nat.accelerometer.get((err, ret) => {\n\tif (err) {\n\t\tNat.toast('[ERROR] ' + JSON.stringify(err))\n\t\treturn\n\t}\n\n\tNat.toast(JSON.stringify(ret, null, 2))\n})")])]), _c('div', {
    staticClass: ["case"],
    on: {
      "click": _vm.watchAcceleration
    }
  }, [_vm._m(1), _c('text', {
    staticClass: ["code"]
  }, [_vm._v("Nat.accelerometer.watch((err, ret) => {\n\ttimes++\n\n\tif (err) {\n\t\tNat.toast('[ERROR] TIMES: ' + times + '\\n' + JSON.stringify(err))\n\t\treturn\n\t}\n\n\tthis.result = 'TIMES: ' + times + '\\n\\nx: ' + ret.x + '\\ny: ' + ret.y + '\\nz: ' + ret.z\n})")])]), (this.result) ? _c('div', {
    staticClass: ["case"]
  }, [_c('text', {
    staticClass: ["code"]
  }, [_vm._v(_vm._s(this.result))])]) : _vm._e(), _c('div', {
    staticClass: ["case"],
    on: {
      "click": _vm.clearWatchAcceleration
    }
  }, [_vm._m(2), _c('text', {
    staticClass: ["code"]
  }, [_vm._v("Nat.accelerometer.clearWatch((err) => {\n\tif (err) {\n\t\tNat.toast('[ERROR] ' + JSON.stringify(err))\n\t\treturn\n\t}\n\tNat.toast('cleared')\n})")])])], 1)], 1)
  ...
  return _c('div', {
    ...
    staticClass: ["title"]
  }, [_vm._v("Clear watch acceleration")])])
}]}
module.exports.render._withStripped = true
