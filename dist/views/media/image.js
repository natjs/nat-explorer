...
__vue_options__.__file = "/Users/acathur/Developments/nat/nat-explorer/src/views/media/image.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-65f971be"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
	...
	methods: {
		pick: function pick() {
			...
			_natjs2.default.image.pick({
				...
				_this.path = ret.paths[0];
			});
		},
		preview: function preview() {
			_natjs2.default.image.preview(['http://cdn.instapp.io/nat/samples/bnw.jpeg', 'http://cdn.instapp.io/nat/samples/fibonacci.jpeg'].concat(this.path));
		},
		info: function info() {
			_natjs2.default.image.info(this.path, function (err, ret) {
				if (err) {
					_natjs2.default.toast('[ERROR] ' + JSON.stringify(err));
					return;
				}
		...
		exif: function exif() {
			_natjs2.default.image.exif(this.path, function (err, ret) {
				if (err) {
					_natjs2.default.toast('[ERROR] ' + JSON.stringify(err));
					return;
				}
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('status-bar'), _c('nav-bar'), _c('scroller', {
    ...
    staticClass: ["code"]
  }, [_vm._v("Nat.image.info(this.path, (err, ret) => {\n\tif (err) {\n\t\tNat.toast('[ERROR] ' + JSON.stringify(err))\n\t\treturn\n\t}\n\n\tNat.toast({\n\t\tmessage: JSON.stringify(ret, null, 2),\n\t\tduration: 7000\n\t})\n})")])]), _c('div', {
    staticClass: ["case"],
    on: {
      "click": _vm.exif
    }
