...
__vue_options__.__file = "/Users/acathur/Developments/nat/nat-explorer/src/views/media/audio.vue"
__vue_options__.render = __vue_template__.render
__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
__vue_options__._scopeId = "data-v-b4eea28e"
__vue_options__.style = __vue_options__.style || {}
__vue_styles__.forEach(function (module) {
  for (var name in module) {
    __vue_options__.style[name] = module[name]
...
exports.default = {
	...
	data: function data() {
		return {
			name: 'Audio',
			path: 'http://cdn.instapp.io/nat/samples/audio.mp3'
		};
	},
	...
	methods: {
		play: function play() {
			_natjs2.default.audio.play(this.path, function (err) {
				if (err) {
		...
		pause: function pause() {
			_natjs2.default.audio.pause(function (err) {
				if (err) {
					_natjs2.default.toast('[ERROR] ' + JSON.stringify(err));
					return;
				}
		...
		stop: function stop() {
			_natjs2.default.audio.stop(function (err) {
				if (err) {
					_natjs2.default.toast('[ERROR] ' + JSON.stringify(err));
					return;
				}
...
module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
  return _c('div', [_c('status-bar'), _c('nav-bar'), _c('scroller', {
    ...
    staticClass: ["code"]
  }, [_vm._v("Nat.audio.play(this.path, (err) => {\n\tif (err) {\n\t\tNat.toast('[ERROR] ' + JSON.stringify(err))\n\t\treturn\n\t}\n\n\tNat.toast('started')\n})")])]), _c('div', {
    staticClass: ["case"],
    on: {
      "click": _vm.pause
    }
